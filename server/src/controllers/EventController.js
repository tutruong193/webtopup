...
const createEvent = async (req, res) => {
  try {
    ...
    if (
      firstCloseDateSau.getTime() - openDateSau.getTime() < 0 ||
      finalCloseDateSau.getTime() - openDateSau.getTime() < 0 ||
      finalCloseDateSau.getTime() - firstCloseDateSau.getTime() < 0
    ) {
      // Sửa biến
      return res.status(200).json({
        status: "ERR",
    ...
    const finalCloseDateVietNam =
      moment(finalCloseDateSau).tz("Asia/Ho_Chi_Minh"); // Thêm biến
    if (
      !openDateVietNam.isAfter(currentTimeVietnam) ||
      !firstCloseDateVietNam.isAfter(currentTimeVietnam) ||
      !finalCloseDateVietNam.isAfter(currentTimeVietnam)
    ) {
      // Sửa biến
      return res.status(200).json({
        status: "ERR",
